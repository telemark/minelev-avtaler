...
module.exports = async options => {
  ...
  const repack = samtykke => {
    const student = options.students.find(student => samtykke.userId === student.sam)
  ...
  if (options.students.length > 0) {
    const sams = options.students
      .filter(student => Object.prototype.isPrototypeOf.call(student, 'sam'))
      .map(student => student.sam)
    const payload = {
      userIds: sams
    }
    try {
